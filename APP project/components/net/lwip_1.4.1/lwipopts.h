...
*
*\File          lwipopts.h
*\Description   lwip配置头文件，一些配置转移到了rtconfig.h文件中
*\Note          
*\Log           2015.04.07    Ver 1.0    张波
*               创建文件。
***************************************************************************************************/
...
#define TCP_KEEPIDLE_DEFAULT                30000   //保活时间 30s 如该连接在60秒内没有任何数据往来,则进行探测
#define TCP_KEEPINTVL_DEFAULT               3000   //保活探测 5s 探测时发包的时间间隔为5 秒
#define TCP_KEEPCNT_DEFAULT                 3U      // 探测尝试的次数。如果第1次探测包就收到响应了,则后2次的不再发。
...
#define UDP_TTL                     255
#define DEFAULT_UDP_RECVMBOX_SIZE   4 //ptpd协议使用时，此值大一点，否则接收不及时会造成丢包,此值与PTP从机规模相关

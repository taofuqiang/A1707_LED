...
UINT32 portTICK_RATE_MS = 1000/RT_TICK_PER_SECOND;

INT32 aliyun_iot_timer_start_clock(ALIYUN_IOT_TIME_TYPE_S *timer)
{
    timer->time = rt_tick_get() * portTICK_RATE_MS;
...
INT32 aliyun_iot_timer_spend(ALIYUN_IOT_TIME_TYPE_S *start)
{
    ...
    now = rt_tick_get() * portTICK_RATE_MS;
    res = now - start->time;
    return res;
}
...
{
    return (UINT32) rt_tick_get() * portTICK_RATE_MS;
}

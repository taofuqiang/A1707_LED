...
uint32_t Flash_Storage_Header(uint8_t *pData)
{
    ...
    if (Flash_Erase_Page(addr_appinfo, FLASH_PAGE_SIZE) != HAL_OK)
    {
        return 0;
    }
	...
	Tem = 0x1234567812345678;
		Tem = *((uint64_t *)pData);
	FLASH_WaitForLastOperation((uint32_t)FLASH_TIMEOUT_VALUE);
    if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, addr_appinfo, Tem )!= HAL_OK)
    {
        return 0;
    }
		pData  = pData + 8;
		Tem = *((uint64_t *)pData);
	FLASH_WaitForLastOperation((uint32_t)FLASH_TIMEOUT_VALUE);
    if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, addr_appinfo + 8, Tem ) != HAL_OK)
    {
        return 0;
    }
		pData  = pData + 8;
		Tem = *((uint64_t *)pData);
	FLASH_WaitForLastOperation((uint32_t)FLASH_TIMEOUT_VALUE);
	if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, addr_appinfo + 16, Tem  )!= HAL_OK)
    {
        return 0;
    }
